...
$stmt = $pdo->prepare("SELECT username, current_score FROM users WHERE id = :id LIMIT 1");
$stmt->execute(['id' => $user_id]);
$user = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$user) {
    // User ID in session but no user record found in DB
    // Redirect to register page to create an account
...
$user_score = is_numeric($user['current_score']) ? (int)$user['current_score'] : 0;

// Now you can use $username and $user_score in your page below
?>
...
<head>
  ...
  align-items: center;
  padding: 40px 20px 80px; /* add bottom padding so content isn't hidden behind fixed profile */
  position: relative;
}
...
<script>
  ...
  finishBtn.addEventListener('click', () => {
    ...
    // Send score to server for updating
    ...
    .then(data => {
      if (data.success) {
        // You can keep redirect or just update UI here
